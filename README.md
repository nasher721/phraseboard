...
The test harness runs 5 comprehensive suites:
1. **Smoke suite (`smoke.ahk`)**: Startup and lifecycle verification.
2. **Model suite (`model.ahk`)**: 73 tests verifying trigger normalization, SmartCase, SmartComplete ranking, execution timing, folder hierarchy, cycle rejection, and encrypted PB3 persistence.
3. **Macro suite (`macros.ahk`)**: 25 tests covering AST parsing, nested syntax, relative dates/times, scoped variables, conditionals, bounded loops, safe math evaluations, nested phrase expansion, recursion depth limits, and interactive forms.
4. **Hotkey suite (`hotkeys.ahk`)**: 10 tests verifying hotkey normalization, Mac modifier mapping, system shortcut conflict detection, and runtime registration.
5. **Integration harness (`integration.ahk`)**: 37 end-to-end tests exercising DPAPI encryption, live clipboard capture, RTF formatting, native paste restoration, SmartComplete UI, and safe process termination.
